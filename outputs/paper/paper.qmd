...
library(ggplot2)
### Input data ###
income_percentage<-read.csv(here::here("outputs/data/income_3years.csv"))
...
income_percentage$Income <- 
  factor(income_percentage$Income, 
         levels = income_label)

# Manually set fill colors and labels
fill_colors <- c("2014" = "#6B5B95", "2016" = "#ADD8E6", "2018" =  "#F5E5C0")

ggplot(income_percentage, aes(x = Income, 
                              y = Number, 
                              fill = Year)) +
  geom_bar(stat = "identity", position = "stack") +
  scale_fill_manual(values = fill_colors) +
  labs(x = "Income Level", y = "Total Number of People",
       title = "U.S. Income Distribution by Year") +
  geom_text(aes(label = paste0(round(Percentage, 1), "%"), y = Number + 10), 
